...
| 根据这个 JD 定制简历 | 从你的项目素材里重新选材 → 按 JD 重写 → 生成简历 |
| 帮我优化/改一下这份简历 | 以你现有的简历为基准改写（不从素材库重新选材） |
| 帮我准备这个岗位的面试 | 出题 → 追问预判 → 自我介绍 → 反问清单（不重生简历） |
| 帮我准备投递材料 | 全流程：简历 + 面试准备 |
| 帮我换张照片 | 替换简历照片（旧图自动备份） |
| 打开简历编辑器 | 本地服务 + 浏览器：预览 / 换模板 / 导出 PDF 与长图 |

产出落点：每份简历一个目录 `output/<公司>/<岗位>/`（`resume.md` + `resume.json`，面试准备另存 `interview_prep.md`）。
...
# 2. 初始化（查环境 → 装依赖 → 从模板生成数据文件 → 生成编辑器启动器 → 装 git 钩子）
...
# 只看匹配度，不写简历
分析一下这个岗位适不适合我。JD：{粘贴 JD}

# 准备面试
帮我准备这个岗位的面试。目标公司：{公司全称}，JD：{粘贴 JD}
```

不必每次跑全流程 —— 说清你要哪一段即可（清单见上方「它能为你做什么」）。
...
**按意图分步调用** —— 不必每次跑全流程（能力清单见上方「它能为你做什么」）。

### （可选）可视化编辑器

编辑器是**本地服务 + 浏览器**，跑起来就有 URL——不用关心 skill 装在哪。

| 方式 | 怎么用 |
|---|---|
| **让 AI 打开（推荐）** | 说一句「打开简历编辑器」；或发带参数的 skill 命令：Pi `/skill:resume-pilot 打开编辑器`、Claude Code `/resume-pilot 打开编辑器` |
| 自己打开（skill 安装） | 双击数据目录里的启动器：`~/.resume-pilot/start-editor.bat`（macOS/Linux 用 `start-editor.sh`）——由 `setup.py` 生成，内含 skill 绝对路径 |
| 自己打开（克隆为项目） | 双击项目根目录的 `start-editor.bat`（macOS/Linux 用 `./start-editor.sh`） |
| 命令行（cwd 需在 skill 目录） | `python scripts/start_editor.py` |
...
│   ├── setup.py / init_data.py       # 一键安装 / 初始化数据文件 / 生成编辑器启动器
